...
var loadData = function (message) {
  ...
  for (var z = zmin; z < zmax + 1; z += step) {
    for (var x = xmin; x < xmax + 1; x += step) {
      ...
      //morph
      interleavedArray[i + 3] =
        getMorph(x, z, step, dataView, size) / heightRatio;
      //textures
  ...
  for (var z = zmin; z < zmax + 1; z += step) {
    ...
    //morph
    interleavedArray[i + 3] =
      getMorph(xmin, z, step, dataView, size) / heightRatio - skirtHeight;
    //textures
  ...
  for (var x = xmin; x < xmax + 1; x += step) {
    ...
    //morph
    interleavedArray[i + 3] =
      getMorph(x, zmax, step, dataView, size) / heightRatio - skirtHeight;
    //textures
    interleavedArray[i + 4] = x / size;
  ...
  for (var x = xmin; x < xmax + 1; x += step) {
    ...
    //morph
    interleavedArray[i + 3] =
      getMorph(x, zmin, step, dataView, size) / heightRatio - skirtHeight;
    //textures
    interleavedArray[i + 4] = x / size;
  ...
  for (var z = zmin; z < zmax + 1; z += step) {
    ...
    //morph
    interleavedArray[i + 3] =
      getMorph(xmax, z, step, dataView, size) / heightRatio - skirtHeight;
    //textures
  ...
  var a = {
    x: xmin * sizeRatio + offset.x,
    y: dataView[xmin + zmin * (size + 1)] / heightRatio,
    z: zmin * sizeRatio + offset.z,
  };
  var b = {
    x: xmin * sizeRatio + offset.x,
    y: dataView[xmin + (zmax - step) * (size + 1)] / heightRatio,
    z: zmax * sizeRatio + offset.z,
  };
  var c = {
    x: xmax * sizeRatio + offset.x,
    y: dataView[xmax + (zmax - step) * (size + 1)] / heightRatio,
    z: zmax * sizeRatio + offset.z,
  };
  var d = {
    x: xmax * sizeRatio + offset.x,
    y: dataView[xmax + zmin * (size + 1)] / heightRatio,
    z: zmin * sizeRatio + offset.z,
  };
  ...
  dists.push(
    Math.sqrt(
      Math.pow(mid.x - a.x, 2) +
        Math.pow(mid.y - a.y, 2) +
        Math.pow(mid.z - a.z, 2)
    )
  );
  dists.push(
    Math.sqrt(
      Math.pow(mid.x - b.x, 2) +
        Math.pow(mid.y - b.y, 2) +
        Math.pow(mid.z - b.z, 2)
    )
  );
  dists.push(
    Math.sqrt(
      Math.pow(mid.x - c.x, 2) +
        Math.pow(mid.y - c.y, 2) +
        Math.pow(mid.z - c.z, 2)
    )
  );
  dists.push(
    Math.sqrt(
      Math.pow(mid.x - d.x, 2) +
        Math.pow(mid.y - d.y, 2) +
        Math.pow(mid.z - d.z, 2)
    )
  );
  dists.push(
    Math.sqrt(
      Math.pow(mid.x - l.x, 2) +
        Math.pow(mid.y - l.y, 2) +
        Math.pow(mid.z - l.z, 2)
    )
  );
  dists.push(
    Math.sqrt(
      Math.pow(mid.x - h.x, 2) +
        Math.pow(mid.y - h.y, 2) +
        Math.pow(mid.z - h.z, 2)
    )
  );

  var r = 0;
  for (var i = 0; i < 6; i++) {
    if (dists[i] > r) r = dists[i];
  }
  dists = null;
